...
class Ball(sprite.Sprite):
    ...
    def __init__(self, image_getter, position, direction, is_time_distortion_field_active_getter, resource_manager, attached=False, attach_pos=0):
        sprite.Sprite.__init__(self)
        self.thud_sound_getter = ResourceCapsule(resource_manager, "audio_explosion_dull.wav")
        self.thud_sound_getter().set_volume(0.25)
        ...
        self.is_time_distortion_field_active_getter = is_time_distortion_field_active_getter
    ...
    def update(self, time_passed, paddle, balls, normal_blocks, invulnerable_blocks):
        ...
        """
        if self.is_time_distortion_field_active_getter()() and self.pos.get_y() > 520:
            self.speed = 200.
        else:
    ...
    def combo_done(self):
        if(self.combo > 1):
            self.score += (self.combo + (self.combo_length * 2))
        else:
            ...
            for block in collided_blocks:
                ...
                if not (top_or_bottom_hit or left_or_right_hit):
                    ...
                    if (self.combo_length == 1):
                        ...
                        self.combo += int(score) 
                        self.thud_sound_getter().play()
                    else:
                        score = block.damage(self.damage)
                        self.combo += (int(score)* int(self.combo_length))
                        self.thud_sound_getter().play()
