...
def UserConfirm(screen_getter, resource_manager, message="Are You Sure?", backgroundclass=None):
    ...
    text_widgets = []
    if backgroundclass:
        overlay_sf = pygame.Surface((screen_getter().get_width(), screen_getter().get_height()))
        overlay_sf.fill(Color(10, 10, 10))
        overlay_sf.set_alpha(220)
        screen_getter().blit(overlay_sf, (0,0))
    else: screen_getter().fill((0,0,0))
    ...
    message_text.rect.center = screen_getter().get_rect().center
    message_text.rect.top -= 50
    text_widgets.append(message_text)

    image_buttons = []

    confirmrect = Rect(screen_getter().get_rect().center[0]-96-60, screen_getter().get_rect().center[1]+50, 100, 100)
    confirm_button = ImageButton(screen_getter, resource_manager, ['confirm.png', 'confirm_full.png'], confirmrect, callback=confirm)
    image_buttons.append(confirm_button)

    rejectrect = Rect(screen_getter().get_rect().center[0]+60, screen_getter().get_rect().center[1]+50, 100, 100)
    reject_button = ImageButton(screen_getter, resource_manager, ['reject.png', 'reject_full.png'], rejectrect, callback=reject)
    ...
    while True:
        ...
        for button in image_buttons:
            if button.dirty == True:
                if backgroundclass:
                    ...
                    pygame.draw.rect(screen_getter(), (0,0,0), button.maxrect)
        ...
        """Draw everything"""
        for text in text_widgets:
            text.draw(screen_getter())
        pygame.display.update()


class ImageButton(pygame.sprite.Sprite):
    def __init__(self, screen_getter, resource_manager, image, rect, callback, hover_increase=12):
        """
        Takes 5 arguments:
        ...
        if image.__class__ == "".__class__:
            resource_manager[image] = load_image(image)
            resource_manager["hover" + image] = self.generate_hover_pic(load_image(image)).convert_alpha()
            self.imageset = [ResourceCapsule(resource_manager, image), ResourceCapsule(resource_manager, "hover" + image)]
        elif image.__class__ == self.imageset.__class__ and image[0].__class__ == "".__class__ and image[1].__class__ == "".__class__:
            self.imageset = [ResourceCapsule(resource_manager, "image_" + image[0]), ResourceCapsule(resource_manager, "image_" + image[1])]
        else:
    ...
    def update(self):
        if self.rect.collidepoint(pygame.mouse.get_pos()):
            self.screen_getter().blit(self.imageset[1](), self.rect)
            if self.lastsize != "hover":
                self.dirty = True
                self.lastsize = "hover"
        else:
            self.screen_getter().blit(self.imageset[0](), self.rect)
